...
export default function Home() {
  ...
  function exportFullCsv() {
    ...
    normalizedAccounts.forEach((a) => rows.push({ 数据类型: "账户", 账户ID: a.id, 名称: a.name, 尾号: a.tail, 当前余额: a.balance, 币种: CURRENCY_CODE, 启用: a.active }));
  ...
  return <main className="app-shell">
    ...
    <div className="dashboard">
      ...
      <section className="workspace"><nav className="tabs">{(["收入", "支出", "定投", "资产", "设置"] as const).map((item) => <button className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{item}</button>)}</nav>
        ...
        {tab === "资产" && <div className="tab-panel asset-panel"><div className="panel-head"><div><span>04</span><h2>当前资产</h2></div><p>北京时间今日快照 · 全部人民币</p></div><div className="asset-note">银行卡请填当前人民币实际余额；收支与定投记录不会自动改余额，避免和银行余额重复计算。</div><div className="account-grid">{normalizedAccounts.map((item) => <label className={`account ${item.tone} ${!item.active ? "muted-row" : ""}`} key={item.id}><div><strong>{item.name}</strong><span>{item.tail ? `尾号 ${item.tail}` : "现金账户"}</span></div><div className="balance-input"><span>¥</span><input aria-label={`${item.name}当前人民币实际余额`} type="number" step="0.01" value={item.balance || ""} placeholder="0.00" onChange={(e) => setAccounts(normalizedAccounts.map((account) => account.id === item.id ? { ...account, balance: Number(e.target.value) } : account))} /></div></label>)}</div><div className="holding-grid">{holdings.map((item) => <article key={item.id}><div><strong>{item.name}</strong><span>{item.units.toFixed(4)} 份 × ¥{item.currentPrice || 0}</span></div><strong>{currency.format(item.marketValue)}</strong><small>累计净投入 {currency.format(item.invested)}</small></article>)}</div><label className="market-value"><span>其他理财当前市值（人民币）</span><div><span>¥</span><input type="number" min="0" step="0.01" value={otherInvestmentValue || ""} placeholder="0.00" onChange={(e) => setOtherInvestmentValue(Math.max(0, Number(e.target.value)))} /></div></label><div className="asset-total"><span>当前人民币总资产（余额快照 + 基金市值 + 其他理财）</span><strong>{currency.format(totalAssets)}</strong></div></div>}
