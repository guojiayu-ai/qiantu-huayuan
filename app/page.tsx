...
type SettingsSection = "投资计划" | "账户" | "收支分类" | "备份";
...
export default function Home() {
  ...
  const [settingsSection, setSettingsSection] = useState<SettingsSection>("投资计划");
  ...
  return <main className="app-shell">
    ...
    <div className="dashboard">
      ...
      <section className="workspace"><nav className="tabs">{(["收入", "支出", "定投", "资产", "设置"] as const).map((item) => <button className={tab === item ? "active" : ""} onClick={() => changeTab(item)} key={item}>{item}</button>)}</nav>
        ...
        {tab === "设置" && <div className="tab-panel settings-panel">
          <nav className="settings-tabs" aria-label="设置分区">
            {(["投资计划", "账户", "收支分类", "备份"] as SettingsSection[]).map((item) => <button type="button" key={item} className={settingsSection === item ? "active" : ""} onClick={() => setSettingsSection(item)}>{item}</button>)}
          </nav>
          <div className="settings-view">
            {settingsSection === "投资计划" && <div className="investment-settings"><section className="plan-card"><h3>我的计划</h3><div className="compact-grid"><label>投资目标<input value={plan.goal} onChange={(e) => patchPlan("goal", e.target.value)} /></label><label>目标金额（元）<input type="number" min="0" step="0.01" value={plan.targetAmount || ""} onChange={(e) => patchPlan("targetAmount", Number(e.target.value))} /></label><label>应急金（月）<input type="number" min="0" value={plan.emergencyMonths} onChange={(e) => patchPlan("emergencyMonths", Number(e.target.value))} /></label><label>每月预算（元）<input type="number" min="0" step="0.01" value={plan.monthlyBudget} onChange={(e) => patchPlan("monthlyBudget", Number(e.target.value))} /></label><label>执行频率<select value={plan.frequency} onChange={(e) => patchPlan("frequency", e.target.value)}><option>每周</option><option>双周</option><option>每月</option></select></label><label>固定执行日<input type="number" min="1" max="28" value={plan.executionDay} onChange={(e) => patchPlan("executionDay", Number(e.target.value))} /></label><label>股票目标占比 %<input type="number" min="0" max="100" value={plan.stockTarget} onChange={(e) => patchPlan("stockTarget", Number(e.target.value))} /></label><label>可承受回撤 %<input type="number" min="0" max="100" value={plan.maxDrawdown} onChange={(e) => patchPlan("maxDrawdown", Number(e.target.value))} /></label></div><details><summary><span>买入、暂停、退出与复盘规则</span><small>展开编辑</small></summary><label>买入规则<textarea value={plan.buyRule} onChange={(e) => patchPlan("buyRule", e.target.value)} /></label><label>暂停规则<textarea value={plan.pauseRule} onChange={(e) => patchPlan("pauseRule", e.target.value)} /></label><label>退出规则<textarea value={plan.exitRule} onChange={(e) => patchPlan("exitRule", e.target.value)} /></label><label>复盘频率<input value={plan.reviewFrequency} onChange={(e) => patchPlan("reviewFrequency", e.target.value)} /></label></details></section><section className="fund-card"><div className="card-title"><h3>定投标的</h3><span>支持添加多只</span></div><form className="fund-form" onSubmit={addFund}><label className="fund-name">标的名称<input placeholder="如：红利ETF易方达" value={newFund.name} onChange={(e) => setNewFund({ ...newFund, name: e.target.value })} required /></label><label>基金 / ETF 代码<input placeholder="如：515180" value={newFund.code} onChange={(e) => setNewFund({ ...newFund, code: e.target.value })} /></label><label>组合定位<select value={newFund.role} onChange={(e) => setNewFund({ ...newFund, role: e.target.value })}><option>核心宽基</option><option>价值或红利</option><option>行业补充</option><option>低风险替代</option></select></label><label className="fund-method">估值方法<select value={newFund.valuationMethod} onChange={(e) => setNewFund({ ...newFund, valuationMethod: e.target.value })}><option>盈利收益率法</option><option>博格公式PE法</option><option>博格公式PB法</option><option>不适用</option></select></label><label>每期金额（元）<input type="number" min="0" step="0.01" placeholder="500" value={newFund.baseAmount} onChange={(e) => setNewFund({ ...newFund, baseAmount: e.target.value })} /></label><label>目标占比（%）<input type="number" min="0" max="100" placeholder="100" value={newFund.targetAllocation} onChange={(e) => setNewFund({ ...newFund, targetAllocation: e.target.value })} /></label><button>添加标的</button></form><div className="fund-list">{funds.length ? funds.map((fund) => <article key={fund.id} className={!fund.active ? "muted-row" : ""}><div><strong>{fund.name}</strong><span>{fund.code || "无代码"} · {fund.role} · {fund.valuationMethod}</span></div><label>当前净值（元）<input type="number" min="0" step="0.0001" value={fund.currentPrice || ""} onChange={(e) => setFunds(funds.map((item) => item.id === fund.id ? { ...item, currentPrice: Number(e.target.value) } : item))} /></label><button className="soft-button" onClick={() => setFunds(funds.map((item) => item.id === fund.id ? { ...item, active: !item.active } : item))}>{fund.active ? "停用" : "启用"}</button></article>) : <div className="empty-card">先添加一只计划中的基金，再记录执行</div>}</div></section></div>}
            {settingsSection === "账户" && <section className="settings-card account-settings-card"><h3>账户</h3><form className="mini-form account-settings-form" onSubmit={addAccount}><input aria-label="账户名称" placeholder="如：浦发银行" value={newAccountName} onChange={(e) => setNewAccountName(e.target.value)} required /><input aria-label="银行卡尾号" placeholder="尾号（可不填）" value={newAccountTail} onChange={(e) => setNewAccountTail(e.target.value.replace(/\D/g, ""))} inputMode="numeric" maxLength={8} /><input aria-label="开始记账时的人民币余额" type="number" min="0" step="0.01" placeholder="起始余额" value={newAccountBalance} onChange={(e) => setNewAccountBalance(e.target.value)} /><button type="submit">新增</button></form><div className="chip-list">{normalizedAccounts.map((item) => <button className={item.active ? "chip active-chip" : "chip"} key={item.id} onClick={() => setAccounts(normalizedAccounts.map((account) => account.id === item.id ? { ...account, active: !account.active } : account))}>{accountLabel(item)} · {item.active ? "使用中" : "已停用"}</button>)}</div></section>}
            {settingsSection === "收支分类" && <section className="settings-card category-settings-card">
              <div className="settings-card-head"><div><h3>收支分类</h3><p>只管理当前类型；切换后另一类会隐藏。</p></div><div className="category-toggle" aria-label="分类类型">{(["收入", "支出"] as const).map((type) => <button type="button" key={type} className={newCategoryType === type ? `active ${type === "收入" ? "income" : "expense"}` : ""} onClick={() => setNewCategoryType(type)}>{type}</button>)}</div></div>
              <form className="category-add-form" onSubmit={addCategory}><input aria-label="分类名称" placeholder={`新增${newCategoryType}分类`} value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} required /><button type="submit">新增{newCategoryType}分类</button></form>
              <div className="category-list">{categories.filter((item) => item.type === newCategoryType).map((item) => <article className={!item.active ? "category-row muted-row" : "category-row"} key={item.id}><div><strong>{item.name}</strong><span>{item.active ? "记账时可选" : "已停用"}</span></div><div className="row-actions"><button type="button" onClick={() => setCategories(categories.map((entry) => entry.id === item.id ? { ...entry, active: !entry.active } : entry))}>{item.active ? "停用" : "启用"}</button><button type="button" className="delete" onClick={() => deleteCategory(item)}>删除</button></div></article>)}</div>
              <p className="category-note">删除只会移除以后记账时的选项，历史收支记录会保留。</p>
            </section>}
            {settingsSection === "备份" && <section className="settings-card backup-card"><h3>本机同步与完整备份</h3><p>{syncStatus === "browser-only" ? <>当前是线上地址，只能读取这个浏览器的数据。需要跨浏览器时，请统一打开 <strong>http://localhost:3000</strong>；数据仍只保存在这台电脑。</> : <>本机同步状态：<strong>{syncLabel}</strong>。Chrome、Safari 等浏览器统一打开 <strong>http://localhost:3000</strong>，即可使用同一份本机数据。</>} CSV 包含账户、分类、全部收支、定投计划、标的和交易；JSON 可一键恢复。</p><div><button onClick={exportFullCsv}>导出完整 CSV</button><button className="soft-button" onClick={exportJson}>备份 JSON</button><button className="soft-button" onClick={() => fileRef.current?.click()}>恢复 JSON</button><input ref={fileRef} type="file" accept="application/json,.json" hidden onChange={importJson} /></div></section>}
          </div>
        </div>}
      </section>
    </div>
