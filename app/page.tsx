...
export default function Home() {
  ...
  const [investAccount, setInvestAccount] = useState("icbc-7956");
  ...
  function jsonBackup() { return JSON.stringify({ version: 2, exportedAt: new Date().toISOString(), accounts: normalizedAccounts, categories, flows, investments, legacyInvestments, funds, plan, otherInvestmentValue }, null, 2); }
  ...
  function renderFlowPanel(type: FlowType) {
    ...
    return <div className="tab-panel"><div className="panel-head"><div><span>{type === "收入" ? "01" : "02"}</span><h2>记录{type}</h2></div><div className="panel-actions"><p>{type === "收入" ? "工资、红包和其他进账分开记" : "选择分类和付款账户，几秒记一笔"}</p><button className="text-button" onClick={() => setBatchOpen(!batchOpen)}>{batchOpen ? "收起" : "批量录入"}</button></div></div>
      <form className="sheet-form flow-form" onSubmit={(event) => addFlow(event, type)}><label>日期<input type="date" value={flowDate} onChange={(event) => setFlowDate(event.target.value)} required /></label><label>分类<select value={category} onChange={(event) => setCategory(event.target.value)}>{availableCategories.map((item) => <option key={item.id}>{item.name}</option>)}</select></label><label>账户<select value={flowAccount} onChange={(event) => setFlowAccount(event.target.value)}>{activeAccounts.map((item) => <option value={item.id} key={item.id}>{accountLabel(item)}</option>)}</select></label><label>金额<input type="number" min="0.01" step="0.01" value={flowAmount} onChange={(event) => setFlowAmount(event.target.value)} placeholder="0.00" required /></label><label>备注<input value={flowNote} onChange={(event) => setFlowNote(event.target.value)} placeholder="可不填" /></label><button type="submit" disabled={!activeAccounts.length || !availableCategories.length}>{editingFlowId ? "保存修改" : `添加${type}`}</button>{editingFlowId && <button type="button" className="soft-button" onClick={() => { setEditingFlowId(null); setFlowAmount(""); setFlowNote(""); }}>取消</button>}</form>
      {batchOpen && <div className="batch-box"><textarea value={batchText} onChange={(event) => setBatchText(event.target.value)} placeholder={`每行一笔：日期,分类,账户,金额,备注\n${today},餐饮,工商银行,25.8,午餐`} /><div><span>支持逗号或从表格复制的制表符；重复记录会自动跳过</span><button onClick={() => importBatch(type)}>检查并导入</button></div></div>}
  ...
  return <main className="app-shell">
    <header className="simple-nav"><div><strong>钱途花园</strong><span>个人资产记录</span><span className="privacy-badge">仅保存在本地</span></div><div className="nav-right"><label>月份<input type="month" value={month} onChange={(event) => setMonth(event.target.value)} /></label><button onClick={exportFullCsv}>导出完整 CSV</button></div></header>
    <div className="dashboard">
      ...
      <section className="workspace"><nav className="tabs">{(["收入", "支出", "定投", "资产", "设置"] as const).map((item) => <button className={tab === item ? "active" : ""} onClick={() => setTab(item)} key={item}>{item}</button>)}</nav>
        ...
        {tab === "定投" && <div className="tab-panel invest-panel"><div className="panel-head"><div><span>03</span><h2>定投计划与执行</h2></div><p>目标 → 规则 → 标的 → 每笔执行</p></div><div className="invest-layout"><section className="plan-card"><h3>我的计划</h3><div className="compact-grid"><label>投资目标<input value={plan.goal} onChange={(e) => patchPlan("goal", e.target.value)} /></label><label>目标金额<input type="number" value={plan.targetAmount || ""} onChange={(e) => patchPlan("targetAmount", Number(e.target.value))} /></label><label>应急金（月）<input type="number" value={plan.emergencyMonths} onChange={(e) => patchPlan("emergencyMonths", Number(e.target.value))} /></label><label>每月预算<input type="number" value={plan.monthlyBudget} onChange={(e) => patchPlan("monthlyBudget", Number(e.target.value))} /></label><label>执行频率<select value={plan.frequency} onChange={(e) => patchPlan("frequency", e.target.value)}><option>每周</option><option>双周</option><option>每月</option></select></label><label>固定执行日<input type="number" min="1" max="28" value={plan.executionDay} onChange={(e) => patchPlan("executionDay", Number(e.target.value))} /></label><label>股票目标占比 %<input type="number" value={plan.stockTarget} onChange={(e) => patchPlan("stockTarget", Number(e.target.value))} /></label><label>可承受回撤 %<input type="number" value={plan.maxDrawdown} onChange={(e) => patchPlan("maxDrawdown", Number(e.target.value))} /></label></div><details><summary>买入、暂停、退出与复盘规则</summary><label>买入规则<textarea value={plan.buyRule} onChange={(e) => patchPlan("buyRule", e.target.value)} /></label><label>暂停规则<textarea value={plan.pauseRule} onChange={(e) => patchPlan("pauseRule", e.target.value)} /></label><label>退出规则<textarea value={plan.exitRule} onChange={(e) => patchPlan("exitRule", e.target.value)} /></label><label>复盘频率<input value={plan.reviewFrequency} onChange={(e) => patchPlan("reviewFrequency", e.target.value)} /></label></details></section><section className="fund-card"><h3>定投标的</h3><form className="fund-form" onSubmit={addFund}><input placeholder="基金名称" value={newFund.name} onChange={(e) => setNewFund({ ...newFund, name: e.target.value })} required /><input placeholder="代码" value={newFund.code} onChange={(e) => setNewFund({ ...newFund, code: e.target.value })} /><select value={newFund.role} onChange={(e) => setNewFund({ ...newFund, role: e.target.value })}><option>核心宽基</option><option>价值或红利</option><option>行业补充</option><option>低风险替代</option></select><select value={newFund.valuationMethod} onChange={(e) => setNewFund({ ...newFund, valuationMethod: e.target.value })}><option>盈利收益率法</option><option>博格公式PE法</option><option>博格公式PB法</option><option>不适用</option></select><input type="number" placeholder="每期金额" value={newFund.baseAmount} onChange={(e) => setNewFund({ ...newFund, baseAmount: e.target.value })} /><input type="number" placeholder="目标占比%" value={newFund.targetAllocation} onChange={(e) => setNewFund({ ...newFund, targetAllocation: e.target.value })} /><button>新增标的</button></form><div className="fund-list">{funds.length ? funds.map((fund) => <article key={fund.id} className={!fund.active ? "muted-row" : ""}><div><strong>{fund.name}</strong><span>{fund.code || "无代码"} · {fund.role} · {fund.valuationMethod}</span></div><label>当前净值<input type="number" step="0.0001" value={fund.currentPrice || ""} onChange={(e) => setFunds(funds.map((item) => item.id === fund.id ? { ...item, currentPrice: Number(e.target.value) } : item))} /></label><button className="soft-button" onClick={() => setFunds(funds.map((item) => item.id === fund.id ? { ...item, active: !item.active } : item))}>{fund.active ? "停用" : "启用"}</button></article>) : <div className="empty-card">先添加一只计划中的基金，再记录执行</div>}</div></section></div>
          <form className="sheet-form trade-form" onSubmit={addInvestment}><label>日期<input type="date" value={investDate} onChange={(e) => setInvestDate(e.target.value)} required /></label><label>操作<select value={investType} onChange={(e) => setInvestType(e.target.value as InvestType)}><option>买入</option><option>卖出</option><option>分红</option><option>费用</option></select></label><label>基金<select value={investFund} onChange={(e) => setInvestFund(e.target.value)}>{activeFunds.map((fund) => <option value={fund.id} key={fund.id}>{fund.name}</option>)}</select></label><label>账户<select value={investAccount} onChange={(e) => setInvestAccount(e.target.value)}>{activeAccounts.map((account) => <option value={account.id} key={account.id}>{accountLabel(account)}</option>)}</select></label><label>金额<input type="number" step="0.01" min="0.01" value={investAmount} onChange={(e) => setInvestAmount(e.target.value)} required /></label><label>份额<input type="number" step="0.0001" min="0" value={investUnits} onChange={(e) => setInvestUnits(e.target.value)} required={investType === "买入" || investType === "卖出"} /></label><label>成交价<input type="number" step="0.0001" min="0" value={investPrice} onChange={(e) => setInvestPrice(e.target.value)} /></label><label>费用<input type="number" step="0.01" min="0" value={investFee} onChange={(e) => setInvestFee(e.target.value)} /></label><label>指数估值<input value={investValuation} onChange={(e) => setInvestValuation(e.target.value)} placeholder="如 PE 12.3" /></label><label>触发规则<input value={investRule} onChange={(e) => setInvestRule(e.target.value)} placeholder="如 月度基准" /></label><label>备注<input value={investNote} onChange={(e) => setInvestNote(e.target.value)} /></label><button disabled={!activeFunds.length || !activeAccounts.length}>{editingInvestmentId ? "保存修改" : "记录执行"}</button>{editingInvestmentId && <button type="button" className="soft-button" onClick={() => { setEditingInvestmentId(null); setInvestAmount(""); setInvestUnits(""); setInvestNote(""); }}>取消</button>}</form><div className="table-wrap trade-table"><table><thead><tr><th>日期</th><th>操作</th><th>基金</th><th>账户</th><th>估值 / 规则</th><th className="number">份额</th><th className="number">金额</th><th /></tr></thead><tbody>{monthInvestments.length ? monthInvestments.map((item) => <tr key={item.id}><td>{item.date}</td><td><span className={`type invest-${item.type}`}>{item.type}</span></td><td>{item.fundName}<small>{item.fundCode}</small></td><td>{resolveAccount(item)}</td><td>{item.valuation || "—"}<small>{item.rule || "未填写触发规则"}</small></td><td className="number">{item.units || "—"}</td><td className="number">{currency.format(item.amount)}</td><td><div className="row-actions"><button onClick={() => editInvestment(item)}>编辑</button><button className="delete" onClick={() => deleteInvestment(item)}>删除</button></div></td></tr>) : <tr><td className="no-data" colSpan={8}>本月暂无定投执行记录</td></tr>}</tbody></table></div></div>}
        {tab === "资产" && <div className="tab-panel asset-panel"><div className="panel-head"><div><span>04</span><h2>当前资产</h2></div><p>今日快照，不受上方历史月份影响</p></div><div className="asset-note">银行卡请填当前实际余额；收支与定投记录不会自动改余额，避免和银行余额重复计算。</div><div className="account-grid">{normalizedAccounts.map((item) => <label className={`account ${item.tone} ${!item.active ? "muted-row" : ""}`} key={item.id}><div><strong>{item.name}</strong><span>{item.tail ? `尾号 ${item.tail}` : "现金账户"}</span></div><div className="balance-input"><span>¥</span><input aria-label={`${item.name}当前实际余额`} type="number" step="0.01" value={item.balance || ""} placeholder="0.00" onChange={(e) => setAccounts(normalizedAccounts.map((account) => account.id === item.id ? { ...account, balance: Number(e.target.value) } : account))} /></div></label>)}</div><div className="holding-grid">{holdings.map((item) => <article key={item.id}><div><strong>{item.name}</strong><span>{item.units.toFixed(4)} 份 × {item.currentPrice || 0}</span></div><strong>{currency.format(item.marketValue)}</strong><small>累计净投入 {currency.format(item.invested)}</small></article>)}</div><label className="market-value"><span>其他理财当前市值</span><div><span>¥</span><input type="number" step="0.01" value={otherInvestmentValue || ""} placeholder="0.00" onChange={(e) => setOtherInvestmentValue(Number(e.target.value))} /></div></label><div className="asset-total"><span>当前总资产（余额快照 + 基金市值 + 其他理财）</span><strong>{currency.format(totalAssets)}</strong></div></div>}
