    ...
    <div className="dashboard">
      ...
      <section className="workspace"><nav className="tabs">{(["收入", "支出", "定投", "资产", "设置"] as const).map((item) => <button className={tab === item ? "active" : ""} onClick={() => changeTab(item)} key={item}>{item}</button>)}</nav>
        ...
        {tab === "定投" && <div className="tab-panel invest-panel"><div className="panel-head"><div><span>03</span><h2>记录投资</h2></div><p>买入、卖出、分红与费用</p></div>
          ...
          <div className="table-wrap trade-table"><table><thead><tr><th>日期</th><th className="trade-operation">操作</th><th className="trade-fund">基金 / 代码</th><th className="trade-account">账户</th><th className="trade-units">份额</th><th className="trade-price">成交价</th><th className="trade-valuation">估值记录</th><th className="trade-amount">成交金额</th><th className="trade-note">备注 / 偏离</th><th /></tr></thead><tbody>{visibleInvestments.length ? visibleInvestments.map((item) => <tr key={item.id}><td>{item.date}</td><td className="trade-operation"><span className={`type invest-${item.type}`}>{item.type}</span></td><td className="trade-fund">{item.fundName}<small>{item.fundCode || "无代码"}</small></td><td className="trade-account">{resolveAccount(item)}</td><td className="trade-units">{item.units ? `${item.units} 份` : "—"}</td><td className="trade-price">{item.price ? currency.format(item.price) : "—"}</td><td className="trade-valuation"><button type="button" className="detail-button" onClick={() => setValuationDetail(item)}>查看详情</button></td><td className="trade-amount">{currency.format(item.amount)}<small>手续费 {currency.format(item.fee)}</small></td><td className="trade-details trade-note"><span>{item.note || "—"}</span><small className="trade-settlement">{item.type === "买入" ? `实际支出 ${currency.format(item.amount + item.fee)}` : item.type === "卖出" ? `实际到账 ${currency.format(Math.max(0, item.amount - item.fee))}` : item.type === "分红" ? `实际到账 ${currency.format(item.amount)}` : `实际支出 ${currency.format(item.amount)}`}</small><small>{item.deviationReason ? `偏离：${item.deviationReason}` : "按计划执行"}</small></td><td><div className="row-actions"><button onClick={() => editInvestment(item)}>编辑</button><button className="delete" onClick={() => deleteInvestment(item)}>删除</button></div></td></tr>) : <tr><td className="no-data" colSpan={10}>{investmentMonth ? "该月暂无定投执行记录" : "暂无定投执行记录"}</td></tr>}</tbody></table></div></div>}
