          ...
          <div className="settings-columns">
            <div className="settings-stack">
              <section className="plan-card">
                ...
                <details><summary><span>买入、暂停、退出与复盘规则</span><small>展开编辑</small></summary><label>买入规则<textarea value={plan.buyRule} onChange={(e) => patchPlan("buyRule", e.target.value)} /></label><label>暂停规则<textarea value={plan.pauseRule} onChange={(e) => patchPlan("pauseRule", e.target.value)} /></label><label>退出规则<textarea value={plan.exitRule} onChange={(e) => patchPlan("exitRule", e.target.value)} /></label><label>复盘频率<input value={plan.reviewFrequency} onChange={(e) => patchPlan("reviewFrequency", e.target.value)} /></label></details>
              </section>
              <section className="settings-card account-card">
                <h3>账户</h3>
                <form className="mini-form account-settings-form" onSubmit={addAccount}><input aria-label="账户名称" placeholder="如：浦发银行" value={newAccountName} onChange={(e) => setNewAccountName(e.target.value)} required /><input aria-label="银行卡尾号" placeholder="尾号（可不填）" value={newAccountTail} onChange={(e) => setNewAccountTail(e.target.value.replace(/\D/g, ""))} inputMode="numeric" maxLength={8} /><input aria-label="开始记账时的人民币余额" type="number" min="0" step="0.01" placeholder="起始余额" value={newAccountBalance} onChange={(e) => setNewAccountBalance(e.target.value)} /><button type="submit">新增</button></form>
                <div className="chip-list">{normalizedAccounts.map((item) => <button className={item.active ? "chip active-chip" : "chip"} key={item.id} onClick={() => setAccounts(normalizedAccounts.map((account) => account.id === item.id ? { ...account, active: !account.active } : account))}>{accountLabel(item)} · {item.active ? "使用中" : "已停用"}</button>)}</div>
              </section>
            </div>
            <div className="settings-stack">
              <section className="fund-card">
                ...
                <div className="fund-list">{funds.length ? funds.map((fund) => <article key={fund.id} className={!fund.active ? "muted-row" : ""}><div className="fund-summary"><strong>{fund.name}</strong><span>{fund.code || "无代码"} · {fund.role} · {fund.valuationMethod}</span></div><label>当前净值（元）<input type="number" min="0" step="0.0001" value={fund.currentPrice || ""} onChange={(e) => setFunds(funds.map((item) => item.id === fund.id ? { ...item, currentPrice: Number(e.target.value) } : item))} /></label><button className="soft-button" onClick={() => setFunds(funds.map((item) => item.id === fund.id ? { ...item, active: !item.active } : item))}>{fund.active ? "停用" : "启用"}</button></article>) : <div className="empty-card">先添加一只计划中的基金，再记录执行</div>}</div>
              </section>
              <section className="settings-card category-card">
                <h3>收支分类</h3>
                <form className="mini-form" onSubmit={addCategory}><select aria-label="分类类型" value={newCategoryType} onChange={(e) => setNewCategoryType(e.target.value as FlowType)}><option>收入</option><option>支出</option></select><input aria-label="分类名称" placeholder={`新增${newCategoryType}分类`} value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} required /><button type="submit">新增</button></form>
