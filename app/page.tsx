    ...
    <div className="dashboard">
      ...
      <section className="workspace"><nav className="tabs">{(["收入", "支出", "定投", "资产", "设置"] as const).map((item) => <button className={tab === item ? "active" : ""} onClick={() => changeTab(item)} key={item}>{item}</button>)}</nav>
        ...
        {tab === "资产" && <div className="tab-panel asset-panel">
          ...
          <div className="asset-note">这里显示截至今天实际拥有的钱。银行卡填当前余额；基金按截至今天持有份额 × 当前净值计算，未来日期交易不会提前计入。</div>
          <div className="account-grid">{normalizedAccounts.map((item) => <label className={`account ${item.tone} ${!item.active ? "muted-row" : ""}`} key={item.id}><div><strong>{item.name}</strong><span>{item.tail ? `尾号 ${item.tail}` : "现金账户"}</span></div><div className="balance-input"><span>¥</span><input aria-label={`${item.name}当前人民币实际余额`} type="number" min="0" step="0.01" value={item.balance || ""} placeholder="0.00" onChange={(e) => setAccounts(normalizedAccounts.map((account) => account.id === item.id ? { ...account, balance: Math.max(0, Number(e.target.value)) } : account))} /></div></label>)}</div>
          ...
          <div className="asset-total"><span>截至今天人民币总资产（当前余额 + 基金当前市值 + 其他理财）</span><strong>{currency.format(totalAssets)}</strong></div>
        </div>}
